...
uint2022_t from_string(const char* buff) {
    ...
    for (uint8_t i = 0; i < (buff_size % 9); ++i) {
        first_nums *= 10;
        first_nums += (buff[i] - '0');
    }

    decimal[buff_size / 9] = first_nums;


    uint16_t cur_ptr = (buff_size % 9);
    while (cur_ptr < buff_size) {
        uint32_t cur_num = 0;

        for (uint16_t i = cur_ptr; i < cur_ptr + 9; ++i) {
            cur_num *= 10;
            cur_num += (buff[i] - '0');
        }

        decimal[(buff_size / 9) - (cur_ptr / 9) - 1] = cur_num;
        cur_ptr += 9;
    }
...
uint2022_t operator-(const uint2022_t& lhs, const uint2022_t& rhs) {
    ...
    for (uint16_t i = 0; i < lhs.get_size() + 1; ++i) {
        if (lhs.bits[i] < rhs.bits[i] + carry) {
            n.bits[i] = 2 + lhs.bits[i] - rhs.bits[i] - carry;
            carry = 1;
        } else {
